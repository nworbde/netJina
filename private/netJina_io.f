...
contains
    ...
    subroutine do_load_nuclib(filename,nuclides,ierr)
        ...
        do
            tmp_n% A(i) = 0.0
            ! try to read a full data line; if that fails, then try to read the 
            ! isotope name; if that fails then exit
            read(nuclib_unitno,fmt=line0_fmt,iostat=ios) &
            & tmp_n% name(i), tmp_n% A(i), tmp_n% Z(i), tmp_n% N(i),  &
            & tmp_n% spin(i), tmp_n% mass_excess(i),tmp_n% provenance(i)
            if (ios == iostat_end) exit
            if (ios /= 0 .or. tmp_n% A(i) == 0.0) cycle
!                 backspace nuclib_unitno
!                 read(nuclib_unitno,fmt=iso_fmt,iostat=ios) iso_name
!                 if (ios /= 0) exit
